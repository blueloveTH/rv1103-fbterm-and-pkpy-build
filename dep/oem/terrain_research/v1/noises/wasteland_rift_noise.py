...
def noise_area(
    bottom_left: vec2i, width: float, height: float, step_size: int, seed: int
) -> array2d[float]:
    voronoi = Voronoi(seed)
    perlin = Perlin(seed)

    # 计算基础网格参数
    cols = max(1, int(width / step_size)+4)
    rows = max(1, int(height / step_size)+4)
    ...
    for layer in layers_config:
        # 生成当前层噪声(使用map向量化计算)
        padded_layer_noise = coords.map(
            lambda pos: generate_layer_noise(pos, layer, perlin=perlin, voronoi=voronoi)
        )
        # 应用层操作(保持padding)
    ...
    result = filter_components_by_area(result, 1, "Moore", 10, 50)
    return result
...
if __name__ == "__main__":
    from array2d import array2d
    from noises.basics.utils import terrain_to_ascii

    result = noise_area(vec2i(-0, 0), 64, 100, 1, base_config["seed"])
