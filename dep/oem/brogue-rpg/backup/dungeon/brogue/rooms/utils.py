...
from ...brogue.const import DIRS_4, ONE, TWO, THREE, FOUR, FIVE, SIX, SEVEN, EIGHT, NINE, ZERO, LevelProfile
from ...brogue.doors import DoorTester
...
def brogue_chooseRandomDoorSites(grid: array2d[int]) -> list[tuple[int, int] | None]:
    ...
    for i, r in enumerate(results):
        if r:
            ret[i] = random.choice(r) # type: ignore
    return ret # type: ignore


def brogue_attachHallwayTo(grid: array2d[int], door_positions: list[tuple[int, int] | None], level_profile: LevelProfile) -> list[tuple[int, int] | None]:
    '''
    ...
    assert grid.width == level_profile.dungeon_width and grid.height == level_profile.dungeon_height
    ...
    for _direction_index, _door_pos in enumerated_door_positions:
        ...
        detect_x = door_x + level_profile.horizontal_corridor_max_length * direction_delta_x
        detect_y = door_y + level_profile.vertical_corridor_max_length * direction_delta_y
    ...
    if direction_index in (0, 1):  # 0,1表示"上,下"方向的direction_index
        corridor_length = random.randint(level_profile.vertical_corridor_min_length, level_profile.vertical_corridor_max_length)
    # 生成水平方向的走廊
    else:
        corridor_length = random.randint(level_profile.horizontal_corridor_min_length, level_profile.horizontal_corridor_max_length)
    
    # 绘制和房间相连的走廊, 走廊的地块使用1表示,和房间地块相同
    direction_delta_x, direction_delta_y = DIRS_4[_direction_index] # type: ignore

    assert door_pos is not None  # 如果上面循环的逻辑没出问题, 那么它不应该是None
    ...
    end_x = clamp(end_x, 0, level_profile.dungeon_width-1)
    end_y = clamp(end_y, 0, level_profile.dungeon_height-1)
