...
class LevelProfile:
    def __init__(
       ...
       # 地牢的总的地图的尺寸, 原作设定是(w:79, h:29)
        ...
        self.horizontal_corridor_min_length = horizontal_corridor_min_length
        self.horizontal_corridor_max_length = horizontal_corridor_max_length
        self.vertical_corridor_min_length = vertical_corridor_min_length
        self.vertical_corridor_max_length = vertical_corridor_max_length
        
        # 房间类型的数量, 原作设定是8, 即一共存在8种样式的房间
        self.room_type_count = room_type_count
        
        # 超大洞穴类型房间的最小尺寸
        self.cavern_min_width = cavern_min_width
        self.cavern_min_height = cavern_min_height
        
        # attachRooms中生成房间的总尝试次数和房间的最大数量, 原作设定是(总尝试次数<=35, 生成的总数量<=35)
        self.attach_rooms_attempt_count = attach_rooms_attempt_count
        self.attach_rooms_max_count = attach_rooms_max_count
        
        # 是否生成墙壁
        self.finish_walls = finish_walls

        self.basic_room_profile = BasicRoomProfile(depth_level, amulet_level)
        self.first_room_profile = FirstRoomProfile(depth_level, amulet_level)
...
class RoomProfile:
    ...
    def _initialize_by_depth(self, depth_level, amulet_level):
        ...
        """
        interpreted_depth = (
            100 * (depth_level - 1) / amulet_level if depth_level > 1 else 0
        )
...
class BasicRoomProfile(RoomProfile):
    """
    地牢在大多数的基本房间的生成概率配置类
    """
...
class FirstRoomProfile(RoomProfile):
    """
    每一层的第一个房间的生成概率配置类
    """
...
class GoblinWarrensProfile(RoomProfile):
    """
    哥布林地洞房间的生成概率配置类
    """
...
class SentinelSanctuariesProfile(RoomProfile):
    """
    哨兵圣所房间的生成概率配置类
    """
